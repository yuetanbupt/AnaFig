...
if __name__ == "__main__":
    ...
    for key, value in dataSet.items():
        ...
        #if 'summarization_only_figure' not in value:
        ...
        #M_sum = str(value['summarization_only_figure'])  # Model-generated summary

        bleu_score_value = compute_bleu(M_sum, G_sum)
        scores_list.append(bleu_score_value)
        valid_samples += 1  
        print(f"Processed {valid_samples} valid samples.")

    
    if valid_samples > 0:
        avg_bleu = average_bleu(scores_list)
        print(f"Average BLEU score: {avg_bleu}")
    else:
        print("No valid samples to compute BLEU scores.")
